...
import net.fishinghacks.utils.client.calc.CustomFunction;
...
import net.fishinghacks.utils.client.calc.parsing.Parser;
    ...
    public void run(String[] args, ChatListener listener) {
        switch (args[0]) {
            ...
            case "craft" -> {
                ...
                calculateCraft(recipe.resultItems(ContextMap.EMPTY).getFirst(), requirements.orElse(List.of()), amount,
                    listener);
            }
            default -> listener.handleSystemMessage(
    ...
    private void calculateCraft(ItemStack result, List<Ingredient> requirements, int amount, ChatListener listener) {
        ...
        for (Ingredient ingredient : requirements) {
            if (ingredient.getValues().size() == 1)
                items.compute(ingredient.getValues().get(0).value(), (i1, count) -> count == null ? 1 : count + 1);
            else ingredient.getValues().unwrap().map(key -> tags.compute(key,
                (i1, count) -> count == null ? new Pair<>(1, ingredient.getValues()) : new Pair<>(count.getA() + 1,
