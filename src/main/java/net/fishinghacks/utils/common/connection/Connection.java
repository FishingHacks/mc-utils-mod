...
public class Connection extends SimpleChannelInboundHandler<Packet<?>> {
    ...
    public Connection(@Nullable Consumer<String> onDisconnect, @Nullable Consumer<Packet<?>> onReceive, boolean isServer, PacketHandler handler) {
    ...
    public String getLoggableAddress() {
        return DualStackUtils.getAddressString(address);
    }

    public String getAddress() {
        if(address instanceof InetSocketAddress a) return a.getHostName() + (a.getPort() == 25560 ? "" : ":" + a.getPort());
    ...
    public void disconnectAndNotify(String reason) {
        ...
        if(isServer && channel.isOpen()) send(new DisconnectPacket(reason), SendListener.thenRun(() -> disconnect(reason)));
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Packet<?> packet) {
        ...
        if (!channel.isOpen()) return;
        if(isServer && (playerName == null || playerId == null) && packet.type().needsLogin()) send(new NotLoggedInPacket());
        else {
