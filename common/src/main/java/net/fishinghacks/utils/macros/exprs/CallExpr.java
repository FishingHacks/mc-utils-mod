...
public record CallExpr(Expression function, List<Expression> arguments, Location location) implements Expression {
    ...
    public LiteralValue eval(EvalContext context) throws MathException, EvalShouldStop {
        ...
        if (callable.type() != LiteralValue.ValueType.BuiltinFunction && callable.type() != LiteralValue.ValueType.Function)
            throw new MathException(Translation.IsNotCallable.with(callable.type().getTranslatedName()), location);
        ...
        try {
            ...
            throw new MathException(Translation.CalledFromNote.get(), location, e);
        }
        assert callable.asFunction().isPresent();
        var function = callable.asFunction().get();
        if (args.size() != function.arguments().size())
            throw new MathException(Translation.MismatchingArguments.with(function.arguments().size(), args.size()),
                location);
        context.enterScope();
