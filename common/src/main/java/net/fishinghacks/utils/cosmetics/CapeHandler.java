...
public class CapeHandler {
    ...
    static void removeProfile(UUID uuid) {
        ...
        if (handler.isAnimated) {
            for (int i = 0; i < handler.maxFrames; ++i)
                manager.release(Constants.id("capes/" + uuid + "/" + i));
        } else {
            manager.release(Constants.id("capes/" + uuid));
        }
    }
    ...
    public ResourceLocation getCape() {
        ...
        if (!this.isAnimated) return Constants.id("capes/" + this.uuid);
        final long time = System.currentTimeMillis();
        if (time > this.lastFrameTime + 100L) {
            this.lastFrame = (this.lastFrame + 1) % this.maxFrames;
            this.lastFrameTime = time;
        }
        return Constants.id("capes/" + this.uuid + "/" + this.lastFrame);
    }
    ...
    private void setCape(NativeImage img, boolean isAnimated) {
        if (isAnimated) {
            ...
            TextureManager manager = Minecraft.getInstance().getTextureManager();
            cape.forEach((frame, texture) -> manager.register(Constants.id(this.nameForFrame(frame)),
            ...
            Minecraft.getInstance().getTextureManager()
                .register(Constants.id("capes/" + this.uuid), new DynamicTexture(() -> "capes/" + this.uuid, cape));
        }
    }
